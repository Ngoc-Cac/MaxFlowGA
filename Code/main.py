...
def LoadSave(entryWidget: tk.Entry, master: tk.Misc = root) -> None:
    ...
    for i, coords in enumerate(positions):
        if i == 0:
            ...
            i = 't'
        try:
            temp_dict[i] = (float(coords.split(',')[0].split('(')[1]),
                            float(coords.split(',')[1].split(')')[0]))
        except ValueError:
    ...
    for i, row in enumerate(temp_matrix):
        for j, num in enumerate(row):
            if num != 0 and (i == len(temp_matrix) - 1 or j == 0):
                ErrorPopUp(master, "Could not read savefile\nPlease check save file for wrong syntax/types/values.")
                return
            elif num != 0:
                temp_graph[i if i != 0 else 's'].append(j if j != len(temp_matrix) - 1 else 't')
    global graph, main_nodes_pos, capacity_matrix
    graph = temp_graph
